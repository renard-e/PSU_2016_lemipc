...
** Last update Sun Apr  2 19:15:13 2017 Rodrigue RENE
*/
...
void			mov_char_map(t_info *info, t_pos const pos_start,
				     t_pos const pos_end);
int			start_ncurses_work(t_info *info, t_global *global);
...
int			init_or_end_ncurses(t_global *global, SCREEN **tmp,
					    int *ret, int indic);
void			init_color_pair();
void			print_color_player(t_pos pos, char *str,
					   int tab);
